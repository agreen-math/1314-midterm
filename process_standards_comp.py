...
\\headerbox{{\\oca}}
\\label{{exact-startoca}}

\\question
Evaluate {func} for the requested values.\\\\
    \\begin{{parts}}
        \\part[3] {parts[0]}\\\\\\vspace{{12pt}} \\answerline
            \\vspace{{\\stretch{{1}}}}
            \\begin{{solution}}
            {sol1}
            \\end{{solution}}
        \\part[3] {parts[1]} \\\\\\vspace{{12pt}} \\answerline
            \\vspace{{\\stretch{{1}}}}
            \\begin{{solution}}
            {sol2}
            \\end{{solution}}
        \\part[3] {parts[2]} \\\\\\vspace{{12pt}} \\answerline
            \\vspace{{\\stretch{{1}}}}
            \\begin{{solution}}
            {sol3}
            \\end{{solution}}
...
def build_q3(item, sols):
    ...
    return f"""%% Combining & Undoing Functions %%
\\headerbox{{\\ocb}}
\\label{{exact-startocb}}
\\question[4]
Evaluate the difference quotient, \\(\\dfrac{{f(x+h)-f(x)}}{{h}}\\), for {func}.\\\\\\vspace{{12pt}}
...
Given {func_f}\\text{{ and }}{func_g}, find the following:\\\\
    \\begin{{parts}}
        \\part[3] \\((f\\circ{{g}})(x)\\) \\\\\\vspace{{12pt}} \\answerline
            \\vspace{{\\stretch{{1}}}}
            \\begin{{solution}}
            {sol1}
            \\end{{solution}}
        \\part[3] \\((f\\circ{{f}})(x)\\) \\\\\\vspace{{12pt}} \\answerline
            \\vspace{{\\stretch{{1}}}}
            \\begin{{solution}}
            {sol2}
            \\end{{solution}}
...
def build_q5(item, sols):
    ...
    sol = sols[0] if sols else ""
    return f"""\\question[5]
Given that {func} is one-to-one, find \\(f^{{-1}}(x)\\).\\\\\\vspace{{12pt}}
...
def build_q6(item, sols):
    func = format_math(extract_func(item))
    sol = sols[0] if sols else ""
    return f"""%% Quadratic Functions %%
\\headerbox{{\\occ}}
\\label{{exact-startocc}}

\\question[5]
Find the vertex, axis of symmetry, x- and y-intercepts, domain, and range for the function.\\\\
...
\\begin{{parts}}
    \\part[\\half] Is the degree of the polynomial even or odd? \\\\\\vspace{{12pt}}

    \\begin{{oneparcheckboxes}}
    \\choice even
    \\choice odd
    \\end{{oneparcheckboxes}}\\\\\\vspace{{.125in}}
    \\part[1] Is the lead coefficient of the polynomial positive or negative?\\\\\\vspace{{12pt}}
 
    \\begin{{oneparcheckboxes}}
    \\choice positive
    \\choice negative
    \\end{{oneparcheckboxes}}\\\\\\vspace{{.125in}}
    \\part[\\half] Is the constant of the polynomial positive or negative? \\\\\\vspace{{12pt}}
    
    \\begin{{oneparcheckboxes}}
    \\choice positive
    \\choice negative
    \\end{{oneparcheckboxes}}\\\\\\vspace{{.125in}}
    \\part[1] What is the minimum degree? \\\\\\vspace{{12pt}}
...
{prompt}\\\\\\vspace{{12pt}}
         ...
         \\hspace{{\\stretch{{1}}}}{item1}: \\fillin[][2in]\\\\\\vspace{{12pt}}
        
         \\hspace{{\\stretch{{1}}}}{item2}: \\fillin[][2in]\\\\
\\begin{{solution}}
{sol}
\\end{{solution}}
\\vspace{{\\stretch{{2}}}}"""
...
def build_q17(item, sols):
    ...
    math = format_math(max(eqs, key=len)) if eqs else ""
    sol = sols[0] if sols else ""
    return f"""\\question[5] Solve the following system using row operations on matrices. Give your answer as an ordered pair. If the system has no solution, write \\textbf{{\\textit{{Inconsistent}}}}\\\\\\
...
\\question[3] Use the table to identify the transformations described by {func}. Circle the option that applies and fill in the blanks as appropriate to describe the transformations on the given function. If one does not apply, you may leave it blank. Then apply these transformations to the graph of the function shown below.\\\\
	\\renewcommand{{\\arraystretch}}{{3}}
	\\begin{{center}}
	\\begin{{tabular}}{{|l|l|}}
		\\hline
...
def build_q19(item, sols):
    ...
    sol = sols[0] if sols else ""
    return f"""\\question[3] Suppose a rational function has the attributes in the table below. Using only the holes, asymptotes, and intercepts listed along with as many of the helpful points as required, sketch the graph of the function.\\\\
\\begin{{solution}}
{sol}
\\end{{solution}}
...
def build_q20(item, sols):
    ...
    return f"""\\question [3] Use the table to identify the transformations described by {func}. Circle the option that applies and fill in the blanks as appropriate to describe the transformations on the given function. If one does not apply, you may leave it blank. Then sketch the graph of the transformed function on the grid below. Indicate any asymptotes with dashed lines.\\\\
...
\newcommand{\exam}{Comprehensive  Exam}
...
def process_checkit_sbg(input_filename, output_filename):
    ...
    print(f"Success! Built {len(output_lines)} customized questions into the SBG Comprehensive template.")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Compile CheckIt LaTeX banks directly into the SBG formatting template.")
    parser.add_argument("--in", "--input", dest="input_file", default="CheckIt_Comp.tex", help="The raw CheckIt output .tex file")
    parser.add_argument("--out", "--output", dest="output_file", default="SBG_Comp_Ready.tex", help="The final generated SBG exam .tex file")
    ...
    try:
        process_checkit_sbg(args.input_file, args.output_file)
